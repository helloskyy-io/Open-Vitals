...
3. **Creates config files** if they don't exist:
   ...
   - `.env` from `templates/.env.template` and **auto-generates** `TEMPORAL_POSTGRES_PASSWORD`, `OPENVITALS_DB_PASSWORD`, and `PGADMIN_DEFAULT_PASSWORD` when creating from template.
...
- **pgAdmin (dev only):** When `temporal.deployment_env` is `dev`, Genesis also starts pgAdmin 4. Port and login email come from **`config.yaml`** → `openvitals.pgadmin.port` (default **5050**) and `openvitals.pgadmin.default_email` (default **admin@local.dev**). Password comes from **`.env`** → `PGADMIN_DEFAULT_PASSWORD` (auto-generated by bootstrap if created from template). After Genesis succeeds, open **http://127.0.0.1:5050** and log in with that email and password. Use pgAdmin to connect to **openvitals-db** (host `openvitals-db`, port 5432, user/db from config, password from `.env` → `OPENVITALS_DB_PASSWORD`) or **temporal-db** (host `temporal-db`, port 5432) as needed.
...
- Step 1: Stop and remove all containers (temporal-*, openvitals-db, jupyter, pgadmin).
